...
public class RadarView extends View {
    ...
    //每个圆圈所占的比例
    private static float[] circleProportion = {1 / 13f, 2 / 13f, 3 / 13f, 4 / 13f, 5 / 13f, 6 / 13f, 7 / 13f};
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        //final int[] colors = {Color.GREEN, Color.BLUE};
        //final float[] positions = {0f, 1f};

        /*SweepGradient gradient = new SweepGradient(mWidth / 2, mHeight / 2, colors, positions);
        float rotate = 45f;
        Matrix gradientMatrix = new Matrix();
        gradientMatrix.preRotate(rotate, mWidth / 2, mHeight / 2);
        gradient.setLocalMatrix(gradientMatrix);
        // 环形颜色填充
        //SweepGradient sweepGradient= new SweepGradient(mWidth / 2, mHeight / 2, colors, null);
        //设置扫描渲染的shader
